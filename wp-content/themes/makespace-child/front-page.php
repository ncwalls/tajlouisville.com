	...
	<?php while( have_posts() ): the_post(); ?>
		<article <?php post_class(); ?> id="post-<?php the_ID(); ?>">
						...
						<?php if($recent_events): ?>
							<ul class="events count-<?php echo count($recent_events); ?>">
								<?php foreach ($recent_events as $event_id): ?>
									<li class="event">
										<a href="<?php echo get_permalink($event_id); ?>" class="inner">
											<?php
												$thumb_image = '';
							...
							<div class="buttons">
								<a href="<?php echo get_post_type_archive_link( 'msw_events' ) ?>" class="button">See Events Calendar</a>
							</div>
						<?php else: ?>
			...
			<aside class="footer-map">
				<?php if(get_field('map_heading') || get_field('map_subheading')): ?>
					<div class="map-header">
						<div class="container">
							<?php if(get_field('map_heading')): ?>
								<h2 class="section-title"><?php the_field('map_heading'); ?></h2>
							<?php endif; ?>
							<?php if(get_field('map_subheading')): ?>
								<h3 class="section-subtitle"><?php the_field('map_subheading'); ?></h3>
							<?php endif; ?>
						</div>
					</div>
				<?php endif; ?>
